...
for i = 1:N
    ...
    c_i = [pos,rand*2*pi,r];
	c(i,:) = c_i;
	I_moyen_disques(i) = calcul_I_moyen_ellipse(I,c_i,c_i(4));
    somme=0;
    for j = 1:N
        dist = sqrt((c(i,1)-c(j,1))^2 + (c(i,2)-c(j,2))^2);
        if dist <= sqrt(2)*(c(i,4)+c(j,4))/2
           somme = somme + 1;
        end
    end
...
while continuer
    ...
    for i=1:nouvN
        ...
        c_i = [nb_colonnes*rand nb_lignes*rand,rand*2*pi,rand*20+5];
        c = [c; c_i];
        I_moyen_disques = [I_moyen_disques ; calcul_I_moyen_ellipse(I,c_i,c_i(4))];
    end
