...
I = double(I);
I = I(1:400,100:450);
figure('Name',['Detection de ' num2str(nb_points_disque) ' flamants roses'],'Position',[0.25*L,0,0.75*L,0.5*H]);
[nb_lignes,nb_colonnes] = size(I);

%% Tirage aléatoire d'une configuration initiale et calcul des niveaux de gris moyens :
c = zeros(N,2);
I_moyen_ellipse = zeros(N,1);
U_ellipse = zeros(N,1);
for i = 1:N
    c_i = nb_colonnes*rand(1,2);
	c(i,:) = c_i;
	I_moyen_disques(i) = calcul_I_moyen(I,c_i,R);
    somme=0;
    for j = 1:N
...
while continuer
    ...
    for i=1:nouvN
        ...
        c = [c; c_i];
        I_moyen_disques = [I_moyen_disques ; calcul_I_moyen(I,c_i,R)];
    end
