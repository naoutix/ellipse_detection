...
for k = 1:k_max
    ...
    %Calcul energie
    Erreur_recouvrement = 0;
    for j = 1:N
        if(i~=j)
            norme = sqrt((c_alea(1) - c(j,1))^2 + (c_alea(2) - c(j,2))^2);
            if(norme <= sqrt(2)*R)
                Erreur_recouvrement = Erreur_recouvrement+1;
            end
        end
    end
    I_moyen_nouv = I_moyen_nouv - beta * Erreur_recouvrement;
